...
namespace JG_Prospect.BLL
{
    public class pdf_BLL
    {
                                     ...
                                     List<Tuple<int, string, int>> proposalOptionList, string downPayment, string Praposal)
        {
            ...
            string resultHeader="",resultBody="",resultBody2="",resultFooter="",categoryBody="";
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                resultBodyA = ds.Tables[0].Rows[0][1].ToString();
            }
            ...
            resultBodyA = resultBodyA.Replace("lblsubmittedto", Customername);
            resultBodyA = resultBodyA.Replace("lblDate", DateTime.Now.Date.ToShortDateString());
            resultBodyA = resultBodyA.Replace("lblhousePhone", customerhousePh);
            resultBodyA = resultBodyA.Replace("lblAddress", customeraddress);
            resultBodyA = resultBodyA.Replace("lblCell", customercellphone);
            resultBodyA = resultBodyA.Replace("lblcitystatezip", citystatezip);
            resultBodyA = resultBodyA.Replace("lblemail", customeremail);
            resultBodyA = resultBodyA.Replace("lblJobLocation", Joblocation);
            resultBodyA = resultBodyA.Replace("lblCustomerId", "C" + CustomerId.ToString());
            resultBodyA = resultBodyA.Replace("width='100%'", "width='700'");
            resultBodyA = resultBodyA.Replace("width='450'", "width='200'");
            ...
            result = resultHeader + resultBodyA;
            ...
            foreach (var prop in proposalOptionList)
            {
                ...
                productid = prop.Item3;
                if (productid != 0 && productid != 1)
                {
                    productid = 7;
                }
                ds = AdminBLL.Instance.FetchContractTemplate(productid);
               ...
               // resultBody = ds.Tables[0].Rows[0][1].ToString();
                ...
                //ds = AdminBLL.Instance.FetchContractTemplate(productid);
                //ds = AdminBLL.Instance.FetchingContractTemplateDetail("Painting");

                //categoryBody = ds.Tables[0].Rows[0][1].ToString();

                result += "<table width='700' align='center'><tr><td width='100%'>" + resultBody + "</td></tr></table>";
                result = result + "<br />";
               
                //if (proposalOptionList.Count > itemCount)
                //{
                //    multipleHeader = ds.Tables[0].Rows[0][0].ToString();
                //    //System.IO.File.WriteAllText("htmlContent.txt", multipleHeader);
                //    string text = System.IO.File.ReadAllText(@"D:\Projects\Justin\JG_Prospect.root\JG_Prospect\JG_Prospect.BLL\htmlContent.txt");
                //    multipleHeader = multipleHeader.Replace(text, string.Empty);
                //    multipleHeader = multipleHeader.Replace(@"""", @"'");
                //    multipleHeader = multipleHeader.Replace(@"src='../img", "src='" + url + @"/img");// @"src="" + url + @""/img");
                //    multipleHeader = multipleHeader.Replace("width='100%'", "width='700'");
                //    multipleHeader = multipleHeader.Replace("width='450'", "width='200'");
                //    multipleHeader = multipleHeader.Replace("url(../img", "url(" + url + "/img");
                //    multipleHeader = multipleHeader.Replace(@"id=""t1""", @"style=""style=""font-family: Verdana, Geneva, sans-serif;      font-size: 8pt;     background: url(http://localhost:49979/img/logo_bg.png) center no-repeat; background-size: 20%;");
                //    result += multipleHeader;
                //    itemCount++;
                //}
            }
            ...
            resultFooter = resultFooter.Replace(@"width=""50%""", @"width=""340""");
        ...
        public string CreateEstimate(string InvoiceNo, int CustomerId, int productType, string totalAmount,
                                     List<Tuple<int, string, int>> proposalOptionList, string downPayment, string Praposal,int prdId)
        {

            int margintop = (prdId+15) % 845;
            margintop = 845 - margintop;// -434;
            string result = string.Empty;
            result = "";
            int totalPages = (prdId + 15+ margintop)/845;
            if (totalPages > 1)
            {
                ...
                //    margintop += count * 10;
                //}
                margintop += 20 * (totalPages - 1);
                margintop = margintop % 845;
            }
            //margintop = margintop % 845;
            string url = ConfigurationManager.AppSettings["URL"].ToString();
            decimal amt2 = 0, amt3 = 0, amt1 = 0;
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                resultBody2 = ds.Tables[0].Rows[0][3].ToString();
                resultFooter = ds.Tables[0].Rows[0][2].ToString();
                resultBodyA = ds.Tables[0].Rows[0][4].ToString();
            }
            CustomerContract contractHeader = new CustomerContract();
            contractHeader = pdf_BLL.Instance.FetchCustomerContractDetails(CustomerId, proposalOptionList[0].Item3, proposalOptionList[0].Item1);
           ...
           int rowspan=proposalOptionList.Count+2;

           result = resultBodyA+"<div style='width:700px'>";// "<html><body><table ><tr><td>" + resultBodyA + "</td></tr>";
           int totalRecord=0;
            foreach (var prop in proposalOptionList)
            {
                totalRecord +=1;
                ...
                //if (totalRecord == proposalOptionList.Count)
                //{
                //    result += "<tr><td style='padding-bottom:421px'><table width='700' align='center'><tr><td width='100%'>" + resultBody + "<br /></td></tr></table></td></tr>";
                //}
                //else
                //{
                //    result += "<tr><td style=''><table width='700' align='center'><tr><td width='100%'>" + resultBody + "<br /></td></tr></table></td></tr>";
                //}
                //"<tr><td style=''><table width='700' align='center'><tr><td width='100%'>" + 
                //</td></tr></table></td></tr>
                result += resultBody + "<br />";
               // result = result + "<br />";
            ...
            resultFooter = resultFooter.Replace(@"width=""100%""", @"width=""700""");
            resultFooter = resultFooter.Replace("url(../img", "url(" + url + "/img");
          //  result += "<tr><td>" + resultBody2 + "</td><td rowspan='" + rowspan + "' style='min-height:900px;max-height:900px;height:1000px;border:0'></td></tr></table>";

            result += "<div style='padding-top:" + margintop + "px'> " + resultBody2 + "</div>";
           // result += resultFooter;
